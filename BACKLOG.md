...
### Add `/ramp:ingest` command — generate topic schemas from external sources
- **Size:** M
- Implement as a new `/ramp:ingest [topic] [source]` command rather than a phase of `/ramp:up` — ingest is a one-time authoring step, not part of the learner flow

...
| Hook stdin contract (JSON shape and exit code semantics) | Can describe the exact stdin shape a hook handler receives: `{ "hook_event_name": "...", "tool_name": "...", "tool_input": {...}, "tool_response": {...} }`; knows exit code semantics: 0 = pass through, 2 = block with stdout as message to Claude; any other exit code = pass through with stderr logged | Qualitative / Artifact | hook handler script with stdin JSON parsing in repo → `[~\|artifact]` | *(empirically verified — 2026-03-09)* |
